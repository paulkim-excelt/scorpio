					...
					struct xhci_hcor *hcor)
{
	...
	int i;
	struct xhci_segment *seg;
	struct xhci_scratchpad *scpad_ptr;
	...
	/* Initializing scratchpad buffers for xhci */
	scpad_ptr = (struct xhci_scratchpad *)
			xhci_malloc(sizeof(struct xhci_scratchpad));
	if (scpad_ptr == NULL) {
		puts("unable to allocate scpad_ptr\n");
		return -ENOMEM;
	}

	/* Allocate 64 bytes of memory for scratchpad buffers */
	for (i = 0; i < MAX_SCR_BUF; i++) {
		scpad_ptr->sp_buf_arr[i] = memalign(SCR_PSZ,
						ALIGN(64, SCR_PSZ));
		BUG_ON(!scpad_ptr->sp_buf_arr[i]);
		memset(scpad_ptr->sp_buf_arr[i], '\0', 64);
	}

	/* Fill DCBAA dev context ptr[0] with scratchpad ptr */
	ctrl->dcbaa->dev_context_ptrs[0] = cpu_to_le64(scpad_ptr);
	debug("%s: Allocated scratch buffers\n", __func__);
