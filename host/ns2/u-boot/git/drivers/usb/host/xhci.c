...
/* The below code needs to be removed since it looks like it's
 * not necessary on actual SVK.
 */
#ifdef CONFIG_PALLADIUM_EMULATION
static int xhci_warm_reset(struct xhci_hcor *hcor, int portnr)
{
	uint32_t val = 0;
	int i = 0;
	volatile uint32_t *status_reg;

	debug("\nStarting warm reset on port %d\n", portnr);

	status_reg = (volatile uint32_t *)
			(&hcor->portregs[le16_to_cpu(portnr) - 1].or_portsc);

	val = xhci_readl(status_reg);
	debug("PORTSC1: 0x%x\n", val);
	val |= (1 << PORTSC_WRC) | (1 << PORTSC_PRC);
	xhci_writel(status_reg, val);

	val = xhci_readl(status_reg);
	val |= (1 << PORTSC_WPR);
	xhci_writel(status_reg, val);

	val |= (1 << PORTSC_PR) | (1 << PORTSC_WPR);
	debug("Writing 0x%x to portsc1\n", val);
	xhci_writel(status_reg, val);
...
#endif /* CONFIG_PALLADIUM_EMULATION */
/**
...
int _xhci_alloc_device(struct usb_device *udev)
{
	...
	debug("%s: portnum = %d\n", __func__, udev->portnr);

	/* Check TRB_PORT_STATUS event and ack it */
	event = xhci_wait_for_event(ctrl, TRB_PORT_STATUS);
	xhci_acknowledge_event(ctrl);

/* This has to be removed since it looks like the below code
 * is not required on tha actual SVK.
 */
#ifdef CONFIG_PALLADIUM_EMULATION
	/* Issue Warm reset */
	xhci_warm_reset(ctrl->hcor, udev->portnr);
