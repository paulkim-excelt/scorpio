...
#define MIIM_BCM54XX_AUX_CTL 0x18 /* Auxiliary control register */
#define MIIM_BCM54XX_AUXCTL_SHDWSEL_MISC 0x0007
#define MIIM_BCM54XX_AUXCTL_ACTL_SMDSP_ENA 0x0800
			       ...
			       u16 reg, u16 chl, u16 value)
{
	...
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_AUX_CTL,
		  MIIM_BCM54XX_AUXCTL_SHDWSEL_MISC);

	tmp = phy_read(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_AUX_CTL);
	tmp |= MIIM_BCM54XX_AUXCTL_ACTL_SMDSP_ENA;
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_AUX_CTL, tmp);

	tmp = (chl * MIIM_BCM_CHANNEL_WIDTH) | reg;
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_EXP_SEL, tmp);
...
static int bcm_egphy_startup(struct phy_device *phydev)
{
	/* Read the Status (2x to make sure link is right) */
	genphy_update_link(phydev);
	genphy_parse_link(phydev);
...
static void bcm_egphy_afe(struct phy_device *phydev)
{
	/****************************************************
	 * AFE WORKAROUND
	 ****************************************************/
	...
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_AUX_CTL, 0x0c30);
 ...
 * bcm_egphy_laneswap
 *
 * Performs the PHY init for laneswapping.
 * This code uses special registers.
 *********************************************************************/
static void bcm_egphy_laneswap(struct phy_device *phydev)
{
	u16  val;
	uint reg;
...
static int bcm_egphy_config(struct phy_device *phydev)
{
	unsigned int reg;
	ulong lnswp;
	...
	/* Setup read from auxilary control shadow register 7 */
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCMEGPHY_AUXCNTL,
		  MIIM_BCMEGPHY_AUXCNTL_ENCODE(7));
	/* Read Misc Control register and or in Ethernet@Wirespeed */
	reg = bcm5482_read_wirespeed(phydev, MIIM_BCMEGPHY_AUXCNTL);
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCMEGPHY_AUXCNTL, reg);

	/* Initial config/enable of secondary SerDes interface */
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCMEGPHY_SHD,
		  MIIM_BCMEGPHY_SHD_WR_ENCODE(0x15, 0xf));
	...
	/* Enable copper/fiber auto-detect */
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCMEGPHY_SHD,
		  MIIM_BCMEGPHY_SHD_WR_ENCODE(0x1e, 0x201));

	/* Check if Eth laneswap and polarity is required */
	lnswp = getenv_ulong("ethlaneswap", 10, 0);
	printf("ethlaneswap = %lu phydev->addr=%d\n", lnswp, phydev->addr);
	if (lnswp)
		bcm_egphy_laneswap(phydev);

	/* AFE workaround for PHY stability */
	bcm_egphy_afe(phydev);

	udelay(5); /* delay before restarting aneg */
...
static struct phy_driver BCMEGPHY28_driver = {
	.name = "Broadcom BCMEGPHY28",
	.uid = BCM_EGPHY28_PHYID,
	.mask = 0xfffffff0,
	.features = PHY_GBIT_FEATURES,
	.config = &bcm_egphy_config,
	.startup = &bcm_egphy_startup,
	.shutdown = &genphy_shutdown,
};
...
int phy_broadcom_init(void)
{
	...
	phy_register(&BCMEGPHY28_driver);
