...
struct pci_controller *pci_bus_to_hose(int bus)
{
	...
	unsigned long default_domain = 0xffff;

	for (hose = pci_get_hose_head(); hose; hose = hose->next) {
		if (bus >= hose->first_busno && bus <= hose->last_busno) {
			domain = getenv_ulong("pci_domain", 10, default_domain);
			if (domain  == default_domain)
				return hose;
			if (hose->domain != domain)
				continue;
			return hose;
		}
	}
...
struct pci_controller *pci_domain_to_hose(int domain)
{
	struct pci_controller *hose;
	...
	printf("pci_domain_to_hose() failed\n");
	return NULL;
}
