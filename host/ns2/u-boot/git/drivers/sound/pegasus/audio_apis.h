...
#define cpu_rd_single_pegasus(addr, size) *((volatile uint32_t*)addr)

#define cpu_wr_single_pegasus(addr, data, size) \
	do {						\
		*((volatile uint32_t *)(addr)) = (uint32_t) (data);	\
	} while (0)

#define I2S_ADDR(addr)	(addr)
#define cpu_rd_single(addr, size) cpu_rd_single_pegasus(I2S_ADDR(addr), size)
#define cpu_wr_single(addr, data, size) \
	cpu_wr_single_pegasus(I2S_ADDR(addr), data, size)
