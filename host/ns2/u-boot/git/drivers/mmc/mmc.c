...
#endif
	debug("cardtype is %08X\n", cardtype);

	if (cardtype & 0x10)
		mmc->card_caps |= MMC_MODE_HS_200;
...
retry_scr:
	...
	if (!(mmc->cfg->host_caps & MMC_MODE_4BIT)) {
		debug("%s: do not need to switch to 4 bit\n", __func__);
		return 0;
	}
#endif
...
static int mmc_startup(struct mmc *mmc)
{
	...
	if (!IS_SD(mmc) && (mmc->version >= MMC_VERSION_4)) {
		...
		if (err == TIMEOUT) {
			...
			err = mmc_send_ext_csd(mmc, ext_csd);
			if (err) {
				printf("mmc_send_ext_csd error: %d\n", err);
				return err;
			}
		} else if (err) {
			/*
			 * Code below relies on ext_csd being valid -
			 * we cannot continue if it is not.
			 */
			printf("mmc_send_ext_csd error: %d\n", err);
			return err;
		}
		...
		cardtype = ext_csd[EXT_CSD_CARD_TYPE] & 0xff;
		if (cardtype & 0x10) {
			debug("%s: HS200 supported, switch voltage\n",
			      __func__);
			mmc->cfg->ops->set_voltage(mmc, SEL_VOLT_1V8);
		}
#endif
	...
	debug("%s: done with change freq, err = %d\n", __func__, err);
	if (err)
		return err;
	...
	if (IS_SD(mmc)) {
		...
		for (idx = 0; idx < ARRAY_SIZE(ext_csd_bits); idx++) {
			...
			debug("%s: Setting bus width as : %08X\n", __func__,
			      extw);
...
#if defined(CONFIG_PEGASUS) || defined(CONFIG_NS2)
	/* Now that clock is set try for HS200 mode... */
	if ((mmc->card_caps & MMC_MODE_HS_200) & (mmc->cfg->
						  host_caps &
						  MMC_MODE_HS_200)) {
		cardtype = ext_csd[EXT_CSD_CARD_TYPE] & 0xff;
		debug("%s: cardtype is %08X\n", __func__, cardtype);
		...
		if (cardtype & 0x10) {
			...
			if (err) {
				debug("%s: mmc_switch failed : %d\n", __func__,
				      err);
			}

			/* Set and/or clear the TAPs to prepare for Tuning */
			mmc->cfg->ops->set_taps(mmc);
			/* Now change frequency to 200MHz... */
			mmc->tran_speed = 200000000;
			mmc_set_clock(mmc, mmc->tran_speed);
			...
			if (err == -1) {
				debug
				    ("%s: Error during tuning procedure... err\
				     == %d\n", __func__, err);
			} else if (err == 2) {
				debug
				    ("%s: Tuning cycle completed but EXECTUNE\
				     still active...\n", __func__);
			} else {
				debug("%s: Tuning completed successfully...\n",
				      __func__);
			}
		}
...
#if defined(CONFIG_PEGASUS) || defined(CONFIG_NS2)
int mmc_send_tuning_cmd(struct mmc *mmc)
{
	struct mmc_cmd cmd;
	struct mmc_data data;

	/* Switch the frequency */
	cmd.cmdidx = MMC_CMD_TUNING;
	cmd.resp_type = MMC_RSP_R1TUNE;
	cmd.cmdarg = 0;

	if (!(mmc->cfg->host_caps & MMC_MODE_8BIT)) {
		debug("%s HS200 Tuning for 4Bit\n", __func__);
		/* 64 byte tuning block */
		char buff[64];
		...
		debug("%s HS200 Tuning for 8Bit\n", __func__);
		/* 128 byte tuning block for 8 bit mode */
		char buff[128];
		data.dest = buff;
...
int mmc_tuning(struct mmc *mmc)
{
	...
	debug("%s: Tuning Sampling Clock...\n", __func__);

	timeout = 32;
	status = 2;
	while ((status == 2) & (timeout > 0)) {
		mmc->cfg->ops->set_tuning(mmc);
		mmc_send_tuning_cmd(mmc);

		status = mmc->cfg->ops->return_tuning_status(mmc);
