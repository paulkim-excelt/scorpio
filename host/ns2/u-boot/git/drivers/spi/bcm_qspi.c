				  ...
				  unsigned int max_hz, unsigned int mode)
{
	...
	priv->bspi_hw_raf =
	    (volatile struct bcm_bspi_raf *)(QSPI_REG_BASE + 0x100);
	priv->mspi_hw = (volatile struct bcm_mspi_hw *)(QSPI_REG_BASE + 0x200);

#ifndef CONFIG_PALLADIUM_EMULATION
	...
	if (CONFIG_IPROC_BSPI_DATA_LANES != 1 ||
		...
		priv->bspi_hw->bits_per_phase |=
		    cpu_to_le32(CONFIG_IPROC_BSPI_READ_DUMMY_CYCLES);

		/* Command byte for BSPI */
		priv->bspi_hw->cmd_and_mode_byte =
		    cpu_to_le32(CONFIG_IPROC_BSPI_READ_CMD);
	...
	/* Check for Palladium what is the apb_clk used */
	spbr = (266000000 - 1) / (2 * priv->max_hz) + 1;
	priv->mspi_hw->spcr0_lsb =
	    cpu_to_le32(max(min(spbr, SPBR_MAX), SPBR_MIN));
...
static int bcmspi_disable_bspi(struct bcmspi_priv *priv)
{
	...
	if ((priv->bspi_hw->mast_n_boot_ctrl & cpu_to_le32(1)) == 0) {
		...
		while (get_timer(start) <
		       QSPI_WAIT_TIMEOUT * CONFIG_SYS_HZ / 1000) {
			if ((priv->bspi_hw->busy_status & cpu_to_le32(1)) == 0) {
				priv->bspi_hw->mast_n_boot_ctrl =
				    cpu_to_le32(1);
				udelay(1);
				break;
			}
			udelay(1);
			priv->bspi_hw_raf->ctrl = cpu_to_le32(1);
		}
...
static void bspi_set_4byte_mode(struct bcmspi_priv *priv, int enable)
{
	...
	if (enable) {
		...
		priv->bspi_hw->bits_per_phase |=
		    cpu_to_le32(BSPI_BITS_PER_PHASE_ADDR_MARK);
...
static void bspi_read_via_raf(struct bcmspi_priv *priv, u8 *rx, uint bytes)
{
	...
	priv->bspi_hw_raf->start_address =
	    cpu_to_le32(priv->bspi_addr & 0x00FFFFFF);
			...
			const u8 *tx, u8 *rx, uint bytes, unsigned long flags)
{
	...
	udelay(1000); /*temporary fix. will remove it*/
	return 0;
}
...
mspi_xfer(struct bcmspi_priv *priv, uint bytes, const u8 *tx, u8 *rx, int end)
{
	if (bytes & 1) {
		...
		if (priv->mspi_16bit) {
			priv->mspi_hw->spcr0_msb |=
			    cpu_to_le32(MSPI_SPCR0_MSB_BITS_8);
			priv->mspi_16bit = 0;
		}
	} else {
	...
	while (bytes) {
		...
		if (priv->mspi_16bit) {
			...
			chunk =
			    min(bytes,
				(uint) NUM_CDRAM * (priv->mspi_16bit ? 2 : 1));
			queues = (chunk - 1) / 2 + 1;
			bytes -= chunk;
			...
			/* Fill TXRAMs */
			for (i = 0; i < chunk; i++) {
				priv->mspi_hw->txram[i] =
				    cpu_to_le32(tx ? tx[i] : 0xff);
			}
