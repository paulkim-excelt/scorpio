...
#ifdef __aarch64__
	/* This address an issue with multi-image: since mkimage only align on
	 * 4-bytes boundary.  When using the dtb, it's a possibility that the
	 * DTB address is not aligned on a 8-bytes boundary which would then
	 * generate a fatal execption. Note that we NEED to use fdt_high set
	 * with a value different from 0xFFFFFFFF
	 */
	for (;;) {
		uint64_t e = (uint64_t)_fdt_mem_rsv(fdt, i);
		e += offsetof(struct fdt_reserve_entry, size);
		if (e % 8) {
			uint64_t a = *(uint32_t *)(e);
			uint64_t b = *(uint32_t *)(e + 4);
			if (a | b)
				break;
		} else {
			const struct fdt_reserve_entry *f;
			f = (const struct fdt_reserve_entry *)e;
			if (fdt64_to_cpu(f->size) != 0)
				break;
		}
		i++;
	}
