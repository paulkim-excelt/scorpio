...
/*weak, platforms can override this function with it's own implementation */
__weak void show_post_board_banner(int flags)
{
	if (flags & POST_AUTO) {
		printf("####################################################");
		printf("\n\t Board:%s, %s\n", getenv("board_name"),
	...
	if ((flags & test_flags & POST_ALWAYS) &&
		...
		if (!(flags & POST_REBOOT)) {
			if ((test_flags & POST_REBOOT) &&
			    !(flags & POST_MANUAL)) {
				post_bootmode_test_on((gd->
						       flags & GD_FLG_POSTFAIL)
						      ? POST_FAIL_SAVE | i : i);
			}
		...
		if (test_flags & POST_PREREL) {
			...
			post_result = (*test->test) (flags);
			if (post_result == 0) {
				post_log("PASSED\n");
				show_post_progress(i, POST_AFTER, POST_PASSED);
			} else if (post_result == BCM_NO_IP) {
				post_log("N/A(NO IP)\n");
				show_post_progress(i, POST_AFTER, POST_PASSED);
			} else {
				post_log("FAILED\n");
		...
		if ((flags & POST_AUTO) && (test_flags & POST_AUTO)) {
			result = (*test->test) (flags);
			return result;
		} else {
			return -2;	/*skipped */
		}
...
int post_run(char *name, int flags)
{
	...
	if (flags & POST_AUTO) {
		for (i = 0; i < POST_MAX_NUMBER; i++)
			test_results[i] = 0xdeadbead;
	}

	post_get_flags(test_flags);

	/* Platform specified banner message! */
	show_post_board_banner(flags);
	...
	if (name == NULL) {
		...
		for (i = 0; i < post_list_size; i++) {
			if ((flags & test_flags[i]) & POST_AUTO) {
				if (test_results[i] == 0)
					post_log("\n%s\t\t\t...Passed!",
						 post_list[i].name);
					...
					post_log("\n%s\t\t\t...Failed!",
						 post_list[i].name);
				else if (test_results[i] == BCM_NO_IP)
					post_log("\n%s\t\t\t...N/A(NO-IP)!",
						 post_list[i].name);
				else
					post_log("\n%s\t\t\t...Failed!",
