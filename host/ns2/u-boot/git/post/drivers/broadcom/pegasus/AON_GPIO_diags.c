...
#define MAX_AON_GPIO	5
#define LED_DEBUG 0

#define rd(x) smc_mem_read32(x)
#define wr(v, x) smc_mem_write32(x, v)
...
static void aon_gpio_data_high(int gpio)
{
	...
	reg_val = rd(GP_DATA_OUT);
	reg_val = reg_val | (1 << gpio);

	wr(reg_val, (GP_DATA_OUT));
#if LED_DEBUG
...
int AON_GPIO_post_test(int flags)
{
	...
	if (s) {
		if (!strcmp(s, PEGASUS_XMC_BOARD)) {
			post_log("\nAON GPIO is not available on XMC\n");
			return BCM_NO_IP;
		}
	}
...
#endif
	if (val != 0x3F) {
		aon_gpio_cfg_output();	/*wr(GP_OUT_EN, 0x3F); */
		val = rd(GP_OUT_EN);
#if LED_DEBUG
	...
	post_log("\nEnsure DIP switch SW12 is ON for all LEDs.\n");
	post_log("\nConfirm to start the test (Y/N):\n");
	do {
		ch = (char)serial_getc();
	...
	post_log("\nAre the #5 AON GPIO Led's ON? (Y/N)\n");
	do {
		ch = (char)serial_getc();
	} while ((ch != 'y') && (ch != 'Y') && (ch != 'n') && (ch != 'N'));
	...
	post_log("\nAre the #5 AON GPIO Led's OFF? (Y/N)\n");
	do {
		ch = (char)serial_getc();
	} while ((ch != 'y') && (ch != 'Y') && (ch != 'n') && (ch != 'N'));
...
#endif
	wr(def_reg_val, (GP_DATA_OUT));
	wr(def_aon_control3_val, (AON_GPIO_CONTROL3));
