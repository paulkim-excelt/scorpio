...
{
	return memory_post_test_patterns(start, size);
}
#endif
 ...
 */
__attribute__((weak))
int arch_memory_test_prepare(unsigned long *vstart, unsigned long *size, phys_addr_t *phys_offset)
{
	bd_t *bd = gd->bd;

	/**vstart = CONFIG_SYS_SDRAM_BASE;*/
	/**size = gd->ram_size - (64 * 1024 * 1024);*/	/* Leave top 64MB for U-boot */
	*vstart = 0x85000000; /* exclude secure DDR partitions */
	*size = gd->ram_size - (144 * 1024 * 1024);	/* Exclude 64MB u-boot + 80MB secure area */

	/* Limit area to be tested with the board info struct */
	if ((*vstart) + (*size) > (ulong)bd)
		*size = (ulong)bd - *vstart;

	post_log ("Memory Range selected: start:0x%lx, end:0x%lx\n", *vstart, *vstart + *size - 1);
 ...
 */
#define NS2_DDR3_HIGH_START		0x100000000ULL
#define NS2_DDR3_HIGH_SIZE		0x80000000ULL

int arch_memory_test_prepare_high(unsigned long *vstart, unsigned long *size, phys_addr_t *phys_offset)
{
	*vstart = NS2_DDR3_HIGH_START;
	*size = NS2_DDR3_HIGH_SIZE;	/* RANK-1 */
...
{
	post_log("\n -----------------------\n");
	post_log("| MEMORY DIAG HELP MENU |\n");
	post_log(" -----------------------\n");
}
...
int memory_post_test(int flags)
{
	...
	arch_memory_test_prepare(&vstart, &memsize, &phys_offset);
	arch_memory_test_prepare_high(&vstart_high, &memsize_high, &phys_offset_high);

	retval1 = memTestDataBus((volatile datum*)vstart);
	if (retval1 != 0)
		return -1;

	post_log("\n\n**** DDR WALK TEST: 2GB RANK-0 ****\n\n");
	retval2 = memTestAddressBus((volatile datum*)vstart, memsize);
	if (retval2 != NULL)
		return -1;

	post_log("\n\n**** DDR WALK TEST: 2GB RANK-1 ****\n\n");
	retval2 = memTestAddressBus((volatile datum*)vstart_high, memsize_high);
	if (retval2 != NULL)
		return -1;

	post_log("\n\n**** DDR FULL TEST: 2GB RANK-0 ****\n\n");
	retval2 = memTestDevice((volatile datum*)vstart, memsize);
	if (retval2 != NULL)
		return -1;

	post_log("\n\n**** DDR FULL TEST: 2GB RANK-1 ****\n\n");
	retval2 = memTestDevice((volatile datum*)vstart_high, memsize_high);
	if (retval2 != NULL)
		return -1;
