...
int fdt_fixup_memory_banks(void *blob, u64 start[], u64 size[], int banks)
{
	...
	if (fdt_getprop(blob, nodeoffset, "reg", &len) && (len > 0))
		/*
		 *  If there is already a memory node with a "reg" property,
		 *  no matter the contents, do not replace it.  This means we
		 *  prefer to trust the DTB memory description over UBoot's
		 *  description, but will fall back to UBoot's description if
		 *  the DTB fails to provide a memory node.
		 */
		return 0;
