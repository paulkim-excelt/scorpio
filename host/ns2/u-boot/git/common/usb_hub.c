...
#if CONFIG_PEGASUS
	/* No external hub support available for xhci yet.
	 * Though external hub is recognized, support is not
	 * available to enumerate all ports.
	 * So, returning at this point.
	 */
	if (dev->devnum > 1) {
		printf("No external USB hub support!!\n");
		return -ENOSYS;
	}
#endif
	return 0;
