...
#define MMC_RSP_R1TUNE	(MMC_RSP_PRESENT)
...
struct mmc_ops {
	...
	void (*set_taps)(struct mmc *mmc);
	int (*set_tuning)(struct mmc *mmc);
	int (*return_tuning_status)(struct mmc *mmc);
	int (*dump_regs)(struct mmc *mmc);
	int (*set_voltage)(struct mmc *mmc, int sel);
#endif
