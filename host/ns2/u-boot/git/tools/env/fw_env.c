			   ...
			   off_t offset, uint8_t mtd_type)
{
	...
	while (processed < count) {
		...
		if (rc != readlen) {
			if (mtd_type == MTD_NORFLASH && rc == DEVESIZE(dev)) {
				processed += rc;
				while (rc > 0) {
					processed += rc;
					readlen -= rc;
	...
	while (processed < write_total) {
		...
		if (mtd_type != MTD_ABSENT) {
			...
			if (mtd_type != MTD_DATAFLASH) {
				if (mtd_type == MTD_NORFLASH) {
					int nb = erasesize / DEVESIZE(dev);
					int n;

					for (n = 0; n < nb; n++) {
						erase.start = blockstart + n *
							      DEVESIZE(dev);
		...
		if (mtd_type == MTD_NORFLASH) {
			...
			for (n = 0; n < nb; n++) {
				if (lseek(fd, blockstart + n * DEVESIZE(dev),
					  SEEK_SET) == -1) {
					fprintf(stderr, "Seek error on %s: %s\n",
						DEVNAME(dev), strerror(errno));
					return -1;
				}
				s = write(fd, data + processed, DEVESIZE(dev));
				if (s != DEVESIZE(dev)) {
					fprintf(stderr, "Write error on %s (s=%d): %s\n",
						DEVNAME(dev), s,
						strerror(errno));
					return -1;
				}
				processed += DEVESIZE(dev);
			}
			break;
		} else {
			if (write(fd, data + processed, erasesize) !=
			    erasesize) {
		...
		if (mtd_type != MTD_ABSENT) {
    		if (mtd_type == MTD_NORFLASH) {
                ...
                int n;
                for (n = 0; n < nb; n++) {
           			erase.start = blockstart + n*DEVESIZE(dev);
   	                erase.length = DEVESIZE(dev);
