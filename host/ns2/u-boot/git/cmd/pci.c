 ...
 * for each device
 */
static void __pciinfo(struct pci_controller *hose,
		      int bus_num, int short_pci_listing)
{
	int device;
	int function;
...
void pciinfo(int bus_num, int short_pci_listing)
{
	...
	__pciinfo(hose, bus_num, short_pci_listing);
}

void pci_domain_info(int domain, int bus_num, int short_pci_listing)
{
	struct pci_controller *hose = pci_domain_to_hose(domain);
...
static int do_pci(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	case 'D':
		if (argc != 5) {
			printf("Usage: pci Domain <domain> <bus> <s/l>\n");
			goto usage;
		}
		domain = simple_strtoul(argv[2], NULL, 16);
		setenv("pci_domain", argv[2]);
		busnum = simple_strtoul(argv[3], NULL, 16);
		if (argv[4][0] == 'l')
			value = 0;
		else
			value = 1;
		pci_domain_info(domain, busnum, value);
		setenv("pci_domain", NULL);
		return 0;
	default:		/* scan bus */
...
static char pci_help_text[] =
	...
	"pci Domain <domain> <bus> [l/s]\n"
	"    - short or long list of PCI devices on domain specific bus\n";
#endif
