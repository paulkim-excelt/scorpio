...
struct blk_desc scsi_dev_desc[CONFIG_SYS_SCSI_MAX_DEVICE];
...
static int scsi_read_capacity(ccb *pccb, lbaint_t *capacity,
			      unsigned long *blksz);
static ulong scsi_read(struct blk_desc *block_dev, lbaint_t blknr, lbaint_t blkcnt,
		       void *buffer);
...
int do_scsi (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    default:
			...
			if (strcmp(argv[1],"read") == 0) {
				...
				n = scsi_read(scsi_get_dev(scsi_curr_dev), blk, cnt, (ulong *)addr);
				...
				printf("\nSCSI write: device %d block # %ld, "
				       "count %ld ... ",
				       scsi_curr_dev, blk, cnt);
				n = scsi_write(scsi_get_dev(scsi_curr_dev), blk, cnt,
					       (ulong *)addr);
